...
public class InternController {
    ...
    @PostMapping
    public ResponseEntity<Intern> createInternProfile(@RequestBody Intern intern) {
        try {
            Intern savedProfile = internService.save(intern);
    ...
    @GetMapping
    public ResponseEntity<List<Intern>> getAllInternProfiles() {
        try {
            List<Intern> profiles = internService.findAll();
            if (profiles.isEmpty()) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Intern> getInternProfileById(@PathVariable("id") int id) {
        Optional<Intern> profile = internService.findById(id);
    ...
    public ResponseEntity<Intern> getInternProfileByUserId(@PathVariable("userId") int userId) {
        Optional<Intern> profile = internService.findByUserId(userId);
        return profile.map(value -> new ResponseEntity<>(value, HttpStatus.OK))
                .orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }
    ...
    public ResponseEntity<List<Intern>> getInternProfilesByStatus(@PathVariable("status") String status) {
        try {
            List<Intern> profiles = internService.findByStatus(status);
            if (profiles.isEmpty()) {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
            return new ResponseEntity<>(profiles, HttpStatus.OK);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Intern> updateInternProfile(@PathVariable("id") int id,
            @RequestBody Intern internProfile) {
        Optional<Intern> existingProfile = internService.findById(id);

        if (existingProfile.isPresent()) {
            Intern profileToUpdate = existingProfile.get();
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<Intern> partialUpdateInternProfile(@PathVariable("id") int id,
            @RequestBody Intern internProfile) {
        Optional<Intern> existingProfile = internService.findById(id);
